...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        pocz.setOnClickListener {
            ...
            dataPocz.set(Calendar.YEAR, dataPocz[2])
            dataPocz.set(Calendar.MONTH, dataPocz[1] - 1)
            dataPocz.set(Calendar.DAY_OF_MONTH, dataPocz[0])
            if (dataKon[0] != 0) {
                endCalendar.set(Calendar.YEAR, dataKon[2])
                endCalendar.set(Calendar.MONTH, dataKon[1] - 1)
                endCalendar.set(Calendar.DAY_OF_MONTH, dataKon[0])
                kalendarz.setDateBounds(startCalendar, endCalendar)
            }
        }
